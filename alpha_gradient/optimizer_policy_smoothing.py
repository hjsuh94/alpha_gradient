...
class PolicyOptimizer:
    def __init__(self, objective, params):
        ...
        self.params = params
        self.kappa_lst = self.params.kappa_lst
    ...
    def iterate(self, max_iterations):
        ...
        while True:
            self.objective.dynamics.kappa = self.kappa_lst[self.iter - 1]
            print("Current Kappa: {0:05f}".format(
                self.objective.dynamics.kappa))
...
class BCPolicyOptimizer(PolicyOptimizer):
    ...
    def iterate(self, max_iterations):
        ...
        Iterate local descent until convergence.
        """
        theta, cost = super().iterate(max_iterations)
        np.save(self.params.filename + "_alpha.npy", np.array(self.alpha_lst))
