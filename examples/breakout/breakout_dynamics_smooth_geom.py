...
class BreakoutDynamics(DynamicalSystem):
    def __init__(self):
        ...
        self.stiffness = 1.0 * np.power(10.0, 2)
        self.damping = 0.999
    ...
    def compute_sg_batch(self, X_PB_batch):
            ...
            # 1st quad
            quad_1 = torch.logical_and(
                (x_pb > self.pad_width / 2),
                (y_pb > self.pad_height / 2))
            center = torch.tensor([self.pad_width / 2, self.pad_height/2])
            dist_to_center = torch.linalg.norm(
                X_PB_batch[quad_1,:] - center, dim=1)
            dists[quad_1] = dist_to_center - self.padball_radius
            normals[quad_1,:] = (X_PB_batch[
                quad_1,:] - center) / dist_to_center.unsqueeze(1)
            #normals[quad_1,:] = torch.tensor([0.0, 1.0])

            # 2nd quad
            quad_2 = torch.logical_and(
                (x_pb < -self.pad_width / 2),
                (y_pb > self.pad_height / 2))
            center = torch.tensor([-self.pad_width/2, self.pad_height/2])
            dist_to_center = torch.linalg.norm(
                X_PB_batch[quad_2,:] - center, dim=1)
            dists[quad_2] = dist_to_center - self.padball_radius
            normals[quad_2,:] = (X_PB_batch[
                quad_2,:] - center) / dist_to_center.unsqueeze(1)
            #normals[quad_2,:] = torch.tensor([0.0, 1.0])                

            # 3rd quad
            quad_3 = torch.logical_and(
                (x_pb < -self.pad_width / 2),
                (y_pb < -self.pad_height / 2))
            center = torch.tensor([-self.pad_width/2, -self.pad_height/2])
            dist_to_center = torch.linalg.norm(
                X_PB_batch[quad_3,:] - center, dim=1)
            dists[quad_3] = dist_to_center - self.padball_radius
            normals[quad_3,:] = (X_PB_batch[
                quad_3,:] - center) / dist_to_center.unsqueeze(1)                
            #normals[quad_3,:] = torch.tensor([0.0, -1.0])

            # 4th quad
            quad_4 = torch.logical_and(
                (x_pb > self.pad_width / 2),
                (y_pb < -self.pad_height / 2))
            center = torch.tensor([self.pad_width/2, -self.pad_height/2])
            dist_to_center = torch.linalg.norm(
                X_PB_batch[quad_4,:] - center, dim=1)
            dists[quad_4] = dist_to_center - self.padball_radius
            normals[quad_4,:] = (X_PB_batch[
                quad_4,:] - center) / dist_to_center.unsqueeze(1)
            #normals[quad_4,:] = torch.tensor([0.0, -1.0])
    ...
    def dynamics_batch(self, x, u):
        ...
        forcemag_batch = relu_batch(-dists, self.stiffness)
