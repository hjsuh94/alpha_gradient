...
class RobotMotion(ObjectiveFunction):
    def __init__(self, x0, xg, T, dynamics, map, Q, R, Qd):
        self.x0 = x0
        self.xg = xg
        self.T = T
        ...
        self.d = T * dynamics.dim_u # horizon of the problem.
    ...
    def evaluate_batch(self, x, w):
        ...
        x_trj, collision_ind = self.dynamics.rollout_batch(self.x0, u_trj)
        cost = self.evaluate_cost_batch(x_trj, u_trj) + collision_ind * 100.0
        return cost.detach().numpy()
    ...
    def gradient_batch(self, x, w):
        ...
        x_trj, collision_ind = self.dynamics.rollout_batch(self.x0, u_trj)
        cost = torch.sum(self.evaluate_cost_batch(x_trj, u_trj) + collision_ind * 100.0)
        cost.backward()
        return z.grad.detach().numpy()
