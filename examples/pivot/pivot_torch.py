...
class PivotTorch(ObjectiveFunction):
    def __init__(self):
        super().__init__(1)
        self.d = 1
    ...
    def evaluate_batch(self, x, w):
        ...
        return cost_array.squeeze().detach().cpu().numpy()
    ...
    def gradient_batch(self, x, w):
        ...
        z = x + w
        grad_array = torch.zeros(B,1)

        hit_ind = (torch.abs(z) <= self.theta_max).bool()
        miss_ind = (torch.abs(z) > self.theta_max).bool()


        grad_array[hit_ind] = -(2.0 * torch.sin(z) * torch.cos(z))[hit_ind]
        grad_array[miss_ind] = 0.0

        return grad_array.squeeze().detach().cpu().numpy()
